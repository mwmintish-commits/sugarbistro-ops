  ...
  return (
    <div style={modal} onClick={onClose}><div style={mbox} onClick={ev => ev.stopPropagation()}>
      ...
      <div style={{ marginTop: 10, display: "flex", gap: 4, flexWrap: "wrap" }}>
        <button onClick={async () => { const sid = prompt("輸入新門市ID（或從門市列表複製）："); if (sid) { await ap("/api/admin/employees", { action: "update", employee_id: empId, store_id: sid }); alert("已調店"); reload(); } }} style={{ padding: "4px 10px", borderRadius: 4, border: "1px solid #4361ee", background: "transparent", color: "#4361ee", fontSize: 10, cursor: "pointer" }}>{"🔄 調店"}</button>
        <button onClick={async () => { if (!confirm("確定解除LINE綁定？員工需重新輸入綁定碼")) return; await ap("/api/admin/employees", { action: "update", employee_id: empId, line_uid: null }); const d2 = await ap("/api/admin/employees", { action: "generate_bind_code", employee_id: empId }); alert("已解除綁定，新綁定碼：" + (d2.bind_code || "")); reload(); }} style={{ padding: "4px 10px", borderRadius: 4, border: "1px solid #b45309", background: "transparent", color: "#b45309", fontSize: 10, cursor: "pointer" }}>{"🔓 解除LINE綁定"}</button>
