...
export default function StockCountPage() {
  ...
  if (done) return (
    <div style={S.box}>
      ...
      <div style={{ fontSize: 13, color: "var(--text-3)", textAlign: "center", marginTop: 8 }}>{storeName} — {empName}</div>
      {mode === "count" && period === "evening" && <div style={{ fontSize: 12, color: "var(--brand-strong)", textAlign: "center", marginTop: 8 }}>系統已自動比對差異，如有異常將通知管理者</div>}
      <div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: "var(--text-3)" }}>可關閉此頁面</div>
    </div>
  );

  if (loading) return <div style={S.box}><p style={{ textAlign: "center", color: "var(--text-3)" }}>載入中...</p></div>;

  // 進貨模式
  if (mode === "delivery") return (
    <div style={S.box}>
      <h2 style={{ fontSize: 16, fontWeight: 600, marginBottom: 4 }}>📦 進貨登記</h2>
      <div style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 16 }}>{storeName} — {empName}</div>

      {deliveryItems.map((d, idx) => (
        <div key={idx} style={{ display: "flex", gap: 6, marginBottom: 8, alignItems: "flex-end" }}>
          <div style={{ flex: 2 }}>
            {idx === 0 && <label style={{ fontSize: 9, color: "var(--text-3)" }}>品項</label>}
          ...
          <div style={{ flex: 1 }}>
            {idx === 0 && <label style={{ fontSize: 9, color: "var(--text-3)" }}>數量</label>}
            <input type="number" inputMode="decimal" value={d.quantity} onChange={e => { const n = [...deliveryItems]; n[idx].quantity = e.target.value; setDeliveryItems(n); }}
              style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid var(--border)", fontSize: 15, textAlign: "center" }} placeholder="0" />
          </div>
          <button onClick={() => setDeliveryItems(deliveryItems.filter((_, i) => i !== idx))}
            style={{ padding: "6px 8px", border: "none", background: "none", fontSize: 14, cursor: "pointer", color: "var(--danger)" }}>✕</button>
        </div>
      ))}

      <button onClick={addDeliveryLine} style={{ width: "100%", padding: 10, borderRadius: 6, border: "2px dashed #ccc", background: "transparent", fontSize: 13, cursor: "pointer", color: "var(--text-3)", marginBottom: 16 }}>＋ 新增品項</button>

      <button onClick={submitDelivery} disabled={submitting || !deliveryItems.length}
        style={{ width: "100%", padding: 14, borderRadius: 8, border: "none", background: submitting ? "var(--text-3)" : "var(--success)", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer" }}>
  ...
  return (
    <div style={S.box}>
      ...
      <div style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 4 }}>{storeName} — {empName}</div>
      <div style={{ fontSize: 11, color: "var(--brand-strong)", marginBottom: 16 }}>請清點每項庫存的實際數量</div>

      {items.length === 0 ? (
        <div style={{ padding: 20, textAlign: "center", color: "var(--text-3)", fontSize: 13 }}>尚未設定盤點品項，請聯繫總部</div>
      ) : (
        <>
          {cats.map(cat => (
            <div key={cat} style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-3)", marginBottom: 6, padding: "4px 8px", background: "var(--surface-warm)", borderRadius: 4 }}>{cat}</div>
              {items.filter(i => i.category === cat).map(item => (
                <div key={item.id} style={{ display: "flex", alignItems: "center", padding: "8px 0", borderBottom: "1px solid var(--divider)" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 500 }}>{item.name}</div>
                    <div style={{ fontSize: 10, color: "var(--text-3)" }}>單位：{item.unit}{item.par_level > 0 ? " ｜標準：" + item.par_level : ""}</div>
                  </div>
                  <input type="number" inputMode="decimal" value={quantities[item.id] || ""}
                    onChange={e => setQuantities({ ...quantities, [item.id]: e.target.value })}
                    placeholder="0" style={{ width: 70, padding: "8px 4px", borderRadius: 8, border: "2px solid var(--border)", fontSize: 18, textAlign: "center", fontWeight: 600 }}
                    onFocus={e => { e.target.style.borderColor = "var(--brand-strong)"; }} onBlur={e => { e.target.style.borderColor = "var(--border)"; }} />
                </div>
              ))}
            </div>
          ))}
