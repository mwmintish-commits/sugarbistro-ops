  ...
  for (const item of memberJson.data || []) {
    try {
      ...
      const rec = {
        ...
        // 自定義結帳明細（會員系統若有提供 customPayments 陣列就帶過來；沒有則保持空陣列）
        custom_payments: Array.isArray(item.customPayments)
          ? item.customPayments.filter(c => c && c.method && Number(c.amount) > 0).map(c => ({
