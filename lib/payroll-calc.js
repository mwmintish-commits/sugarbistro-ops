...
export async function regeneratePayroll(year, month, opts = {}) {
  ...
  const [schedsAll, otAll, attAll] = await Promise.all([
    supabase.from("schedules")
      .select("employee_id, date, day_type, leave_type, leave_hours, shift_id, break_minutes, comp_source, shifts(start_time, end_time, break_minutes)")
    ...
    // 同時帶 timestamp 用於識別「特休出勤 / 例假日緊急出勤」
    supabase.from("attendances")
      .select("employee_id, type, timestamp, late_minutes, early_leave_minutes")
  ...
  // 讀既有 payroll_records 的 disposition 設定（管理者在「對帳」時設定的）
  // 避免「結算」覆寫掉管理者選擇
  const { data: existingRecs } = await supabase.from("payroll_records")
    .select("employee_id, overtime_disposition, attendance_diff_disposition")
    .in("employee_id", empIds).eq("year", year).eq("month", month);
  const dispMap = (existingRecs || []).reduce((m, r) => { m[r.employee_id] = r; return m; }, {});
  ...
  for (const emp of emps || []) {
    ...
    // 管理者本月加班 disposition（"pay" 轉薪 / "comp" 累積補休 / null 預設按原 comp_type）
    const otDisp = dispMap[emp.id]?.overtime_disposition;
    let otPay = 0, compH = 0, compLeaveAdded = 0;
    if (otDisp === "comp") {
      // 全月加班一律轉補休（時數累積到 comp_balance）
      const totalOtMin = ot.reduce((s, r) => s + Number(r.overtime_minutes || 0), 0);
      compLeaveAdded = Math.round((totalOtMin / 60) * 10) / 10;
    } else if (otDisp === "pay") {
      // 全月加班一律轉現金（即使原本標 comp 也轉）
      otPay = ot.reduce((s, r) => s + Number(r.amount || 0), 0);
    } else {
      // 預設按原 comp_type 處理
      otPay = ot.filter(r => r.comp_type === "pay" || r.comp_converted)
        .reduce((s, r) => s + Number(r.amount || 0), 0);
      compH = ot.filter(r => r.comp_type === "comp" && !r.comp_used && !r.comp_converted)
    ...
    // 該員工本月有 clock_in 的日期集合（用於辨識「特休出勤」「例假日緊急出勤」）
    const clockInDates = new Set(
      (attMap[emp.id] || [])
        .filter(a => a.type === "clock_in" && a.timestamp)
        .map(a => String(a.timestamp).slice(0, 10))
    );

    // 特休出勤（勞基法 39 條）：排了特休但實際出勤 → 同國假，加給 1 天工資
    // 排班為 paid_leave + leave_type=annual，且該日有 clock_in
    const annualAttScheds = allScheds.filter(s =>
      s.day_type === "paid_leave" && s.leave_type === "annual" && clockInDates.has(s.date)
    );
    let annualLeaveAttendancePay = 0;
    for (const s of annualAttScheds) {
      const hrs = hrsOf(s) || 8;
      annualLeaveAttendancePay += emp.monthly_salary
        ? Math.round(dailyRate)
        : Math.round(hourlyRate * hrs * 2);
    }

    // 例假日緊急出勤（勞基法 40 條）：因天災事變停止例假，工資加倍 + 補休一日
    // 排班為 regular_off，且該日有 clock_in（系統正常不會排 regular_off 上班，這是漏網之魚）
    const regularOffAttScheds = allScheds.filter(s =>
      s.day_type === "regular_off" && clockInDates.has(s.date)
    );
    let regularOffAttendancePay = 0;
    for (const s of regularOffAttScheds) {
      const hrs = hrsOf(s) || 8;
      regularOffAttendancePay += emp.monthly_salary
        ? Math.round(dailyRate)
        : Math.round(hourlyRate * hrs * 2);
    }

    // 休息日加給（勞基法 24 條第 2 項）— 實務上只有月薪員工會排到 rest_day 出勤
    // 兼職若被誤排成 rest_day 完全不計（不進 base 也不進 rest_day_pay）
    // 公式：hourlyRate × premium(1/3, 2/3, 5/3) + hourlyRate × hrs = 完整 4/3、5/3、8/3 倍工資（含本薪）
    let restDayPay = 0;
    ...
    let annualLeaveUsedDays = 0; // 扣特休的天數累計（給 leave_balances 用）
    ...
    if (emp.monthly_salary) {
      for (const s of allScheds) {
        ...
        } else if (s.day_type === "paid_leave" && s.leave_type === "holiday_comp") {
          // 國定假日挪移補假 → 扣月薪 或 扣特休（看 schedule.comp_source）
          const hrs = Number(s.leave_hours) > 0 ? Number(s.leave_hours) : 8;
          if (s.comp_source === "annual_leave") {
            // 扣特休餘額（不扣薪），結算後寫到 leave_balances
            annualLeaveUsedDays += hrs / 8;
            leaveDetail += (leaveDetail ? "、" : "") + "國定補假扣特休" + (hrs < 8 ? hrs + "hr" : "1天");
          } else {
            // 預設或 comp_source="salary" → 扣月薪（跟 5/1 加給互抵）
            leaveHours += hrs;
            leaveDeduct += Math.round(hourlyRate * hrs);
            leaveDetail += (leaveDetail ? "、" : "") + "國定補假扣薪" + (hrs < 8 ? hrs + "hr" : "1天");
          }
    ...
    const net = base + otPay + holidayPay + restDayPay + annualLeaveAttendancePay + regularOffAttendancePay - ls - hs - suppHealth + allow - deduct - leaveDeduct - lateDeduct;
    ...
    upserts.push({
      ...
      annual_leave_attendance_pay: annualLeaveAttendancePay,
      regular_off_attendance_pay: regularOffAttendancePay,
      ...
      annual_leave_used_days: annualLeaveUsedDays,
      comp_leave_added_hours: compLeaveAdded,
      late_minutes: lateMin, early_leave_minutes: earlyMin, late_deduction: lateDeduct,
      net_salary: net,
    });
    results.push({ name: emp.name, base, otPay, holidayPay, restDayPay, annualLeaveAttendancePay, regularOffAttendancePay, ls, hs, suppHealth, allow, deduct, leaveDeduct, lateDeduct, lateMin, earlyMin, net, workDays, actualClockDays, discrepancy });
  }

  if (upserts.length > 0) {
    await supabase.from("payroll_records").upsert(upserts, { onConflict: "employee_id,year,month" });

    // 將「補休累積時數」和「扣特休天數」同步到 leave_balances
    // 月底結算時，加班轉補休 → comp_balance 增加；國定補假扣特休 → annual_used 增加
    for (const u of upserts) {
      const annualUsed = Number(u.annual_leave_used_days || 0);
      const compAdded = Number(u.comp_leave_added_hours || 0);
      if (annualUsed > 0 || compAdded > 0) {
        const { data: bal } = await supabase.from("leave_balances")
          .select("id, annual_used, comp_balance").eq("employee_id", u.employee_id).eq("year", year).maybeSingle();
        if (bal) {
          await supabase.from("leave_balances").update({
            annual_used: Number(bal.annual_used || 0) + annualUsed,
            comp_balance: Number(bal.comp_balance || 0) + compAdded,
          }).eq("id", bal.id);
        } else {
          await supabase.from("leave_balances").insert({
            employee_id: u.employee_id, year,
            annual_used: annualUsed, comp_balance: compAdded,
          });
        }
      }
    }
