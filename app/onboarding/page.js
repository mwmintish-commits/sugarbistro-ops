...
export default function OnboardingPage() {
  ...
  const [files, setFiles] = useState({ id_card: null, health_check: null });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (hbData.data && Array.isArray(hbData.data)) setHandbook(hbData.data);
      setLd(false);
  ...
  const goNext = async () => {
    ...
    if (step === 1) { if (!files.health_check || !files.id_card) { setErr("請上傳體檢表和身分證"); return; } }
    ...
    if (step === 4) {
      ...
      const r = await ap("/api/onboarding", {
        ...
        health_check_url: files.health_check, id_card_url: files.id_card,
  ...
  return (
    <Box>
      ...
      {step===1 && <Card title="📄 必要文件上傳">
        ...
        <Up label="🪪 身分證正反面" desc="拍照或掃描" file={files.id_card} onFile={async f=>{setFiles({...files,id_card:await f2b(f)});}} />
      </Card>}

      {step===2 && <>
        <div style={{background:"#fff",borderRadius:10,border:"1px solid #e8e6e1",padding:14,marginBottom:10,maxHeight:350,overflowY:"auto"}}>
          <h3 style={{fontSize:14,fontWeight:600,marginBottom:8,textAlign:"center"}}>📖 員工行為規範與工作守則</h3>
          {(handbook||[]).map((ch,i)=><div key={i} style={{marginBottom:8}}>
            <h4 style={{fontSize:12,fontWeight:600,padding:"3px 6px",borderRadius:4,background:(ch.title||"").includes("零容忍")||(ch.title||"").includes("最高")?"#fde8e8":"#faf8f5",color:(ch.title||"").includes("零容忍")?"#b91c1c":"#333",marginBottom:3}}>{ch.title}</h4>
            {(ch.items||[]).map((item,j)=><div key={j} style={{fontSize:11,lineHeight:1.7,color:"#555",paddingLeft:8}}>{"▸ "+item}</div>)}
          </div>)}
        </div>
        <Card title="">
      ...
      {step===3 && <>
        <div style={{background:"#fff",borderRadius:10,border:"1px solid #e8e6e1",padding:14,marginBottom:10,maxHeight:350,overflowY:"auto"}}>
          <h3 style={{fontSize:14,fontWeight:600,marginBottom:8,textAlign:"center"}}>🏆 季績效獎金制度 v2.0</h3>
          <Sec t="適用範圍" items={["全體正式員工（試用期不納入），以季為單位計算發放。"]} />
          <Sec t="達標率與發放比例" items={["<85%→不發放","85~99%→按比例發放","100~119%→全額","≥120%→加成20%"]} />
          <Sec t="個人分配公式" items={["個人獎金 = 獎金池 × (個人加權工時÷全店合計) × 考核係數","一般員工×1.0，主管×1.2","兼職按實際工時"]} />
          <Sec t="季末考核（100分）" items={["出勤紀律30分","工作完成度30分","服務態度20分","違規紀錄20分"]} />
          <Sec t="考核結果" items={["≥80分→全額","70~79分→50%","<70分→取消"]} />
          <Sec t="發放條件" items={["季度累計不虧損","發放日在職","考核≥70分"]} />
          <Sec t="法律聲明" items={["本獎金屬恩惠性給與，非經常性薪資。公司保有最終裁量權。"]} />
        </div>
        <Card title="">
          <label style={{display:"flex",gap:8,cursor:"pointer"}}>
            ...
            <span style={{fontSize:12,lineHeight:1.6}}>本人已詳閱「季績效獎金制度」並了解計算方式與發放條件</span>
          </label>
        </Card>
      </>}
      ...
      {step===4 && <>
        <Card title="📝 工作合約">
          <div style={{fontSize:12,lineHeight:1.8,color:"#444"}}>
            <p><b>甲方：</b>小食糖 SUGARbISTRO</p>
            <p><b>乙方：</b>{form.signature_name}</p>
            <p><b>門市：</b>{rec?.store_name}</p>
            <p><b>到職日：</b>{rec?.hire_date||new Date().toLocaleDateString("sv-SE")}</p>
            <hr style={{margin:"8px 0",border:"none",borderTop:"1px solid #eee"}} />
            <p>一、乙方同意遵守甲方之員工行為規範與工作守則。</p>
            <p>二、乙方了解並同意季績效獎金制度之計算方式與發放條件。</p>
            <p>三、乙方之薪資、工時、休假依勞動基準法及甲方規定辦理。</p>
            <p>四、乙方同意甲方依法代扣勞健保及所得稅。</p>
            <p>五、本合約自到職日起生效，任一方得依法終止。</p>
          </div>
        </Card>
        <div style={{background:"#fff",borderRadius:10,border:"2px solid #0a7c42",padding:14,marginBottom:10}}>
          <label style={{display:"flex",gap:8,cursor:"pointer",marginBottom:10}}>
            ...
            <span style={{fontSize:12,lineHeight:1.6}}>本人確認資料正確無誤，同意簽署工作合約</span>
          </label>
...
function Sec({t,items}){return <div style={{marginBottom:8}}><h4 style={{fontSize:12,fontWeight:600,padding:"3px 6px",borderRadius:4,background:"#faf8f5",marginBottom:3}}>{t}</h4>{items.map((i,j)=><div key={j} style={{fontSize:11,lineHeight:1.7,color:"#555",paddingLeft:8}}>{"▸ "+i}</div>)}</div>;}

function Up({label,desc,file,onFile}){const ref=useRef();return <div onClick={()=>ref.current?.click()} style={{border:"2px dashed "+(file?"#0a7c42":"#ddd"),borderRadius:10,padding:16,textAlign:"center",marginBottom:10,background:file?"#e6f9f0":"#faf8f5",cursor:"pointer"}}><input ref={ref} type="file" accept="image/*,.pdf" style={{display:"none"}} onChange={e=>{if(e.target.files[0])onFile(e.target.files[0]);}} /><div style={{fontSize:28}}>{file?"✅":"📄"}</div><div style={{fontSize:13,fontWeight:600,marginTop:4}}>{label}</div><div style={{fontSize:10,color:"#888"}}>{file?"已上傳（點擊更換）":desc}</div></div>;}
