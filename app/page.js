...
const ROLE_TABS={admin:["schedules","leaves","shifts","attendance","settings","settlements","deposits","employees"],manager:["schedules","leaves","shifts","attendance","settings","settlements","deposits","employees"],store_manager:["schedules","shifts"]};
const TAB_LABELS={schedules:"📅排班",leaves:"🙋請假",shifts:"⏰班別",attendance:"📍出勤",settings:"⚙️設定",settlements:"💰日結",deposits:"🏦存款",employees:"👥員工"};

function api(u,b,token){const h={"Content-Type":"application/json"};if(token)h["x-admin-token"]=token;return b?fetch(u,{method:"POST",headers:h,body:JSON.stringify(b)}).then(r=>r.json()):fetch(u,{headers:h}).then(r=>r.json());}
...
function LoginPage({onLogin}){
  const[step,setStep]=useState("phone");const[phone,setPhone]=useState("");const[code,setCode]=useState("");const[msg,setMsg]=useState("");const[loading,setLoading]=useState(false);
  const sendCode=async()=>{setLoading(true);setMsg("");const r=await api("/api/auth",{action:"send_code",phone});setLoading(false);if(r.success){setStep("code");setMsg("✅ 驗證碼已發送到你的 LINE");}else setMsg("❌ "+r.error);};
  const verify=async()=>{setLoading(true);setMsg("");const r=await api("/api/auth",{action:"verify",phone,code});setLoading(false);if(r.success){localStorage.setItem("admin_token",r.token);localStorage.setItem("admin_info",JSON.stringify(r));onLogin(r);}else setMsg("❌ "+r.error);};
  return(<div style={{minHeight:"100vh",background:"#faf8f5",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif"}}>
    <div style={{background:"#fff",borderRadius:16,border:"1px solid #e8e6e1",padding:"40px 32px",maxWidth:360,width:"100%",textAlign:"center"}}>
      <div style={{fontSize:36,marginBottom:12}}>🍯</div>
      <h1 style={{fontSize:18,fontWeight:600,marginBottom:4}}>小食糖管理後台</h1>
      <p style={{fontSize:12,color:"#999",marginBottom:24}}>Sugar Bistro Admin</p>
      {step==="phone"&&<div>
        <label style={{fontSize:12,color:"#888",display:"block",textAlign:"left",marginBottom:4}}>手機號碼</label>
        <input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="0912345678" style={{width:"100%",padding:"10px 14px",borderRadius:10,border:"1px solid #ddd",fontSize:15,marginBottom:12,textAlign:"center",letterSpacing:1}} onKeyDown={e=>e.key==="Enter"&&sendCode()}/>
        <button onClick={sendCode} disabled={!phone||loading} style={{width:"100%",padding:"12px",borderRadius:10,border:"none",background:phone&&!loading?"#1a1a1a":"#ccc",color:"#fff",fontSize:14,cursor:"pointer"}}>{loading?"發送中...":"發送驗證碼"}</button>
        <p style={{fontSize:11,color:"#999",marginTop:10}}>驗證碼會發送到你綁定的 LINE</p>
      </div>}
      {step==="code"&&<div>
        <p style={{fontSize:12,color:"#0a7c42",marginBottom:12}}>📱 驗證碼已發送到 LINE</p>
        <label style={{fontSize:12,color:"#888",display:"block",textAlign:"left",marginBottom:4}}>輸入 6 位驗證碼</label>
        <input value={code} onChange={e=>setCode(e.target.value)} placeholder="000000" maxLength={6} style={{width:"100%",padding:"10px 14px",borderRadius:10,border:"1px solid #ddd",fontSize:22,marginBottom:12,textAlign:"center",letterSpacing:8,fontWeight:600}} onKeyDown={e=>e.key==="Enter"&&verify()}/>
        <button onClick={verify} disabled={code.length!==6||loading} style={{width:"100%",padding:"12px",borderRadius:10,border:"none",background:code.length===6&&!loading?"#0a7c42":"#ccc",color:"#fff",fontSize:14,cursor:"pointer"}}>{loading?"驗證中...":"登入"}</button>
        <button onClick={()=>{setStep("phone");setCode("");setMsg("");}} style={{marginTop:8,background:"none",border:"none",color:"#999",fontSize:12,cursor:"pointer"}}>← 重新輸入手機號碼</button>
      </div>}
      {msg&&<p style={{marginTop:10,fontSize:12,color:msg.startsWith("✅")?"#0a7c42":"#b91c1c"}}>{msg}</p>}
    </div>
  </div>);
}
...
export default function Admin(){
  ...
  useEffect(()=>{
    ...
    fetch("/api/auth",{headers:{"x-admin-token":token}}).then(r=>r.json()).then(d=>{
      if(d.authenticated){setAuth({token,...d});}else{localStorage.removeItem("admin_token");localStorage.removeItem("admin_info");}
      setChecking(false);
    }).catch(()=>setChecking(false));
  },[]);

  const logout=()=>{const token=localStorage.getItem("admin_token");if(token)api("/api/auth",{action:"logout",token});localStorage.removeItem("admin_token");localStorage.removeItem("admin_info");setAuth(null);};

  if(checking)return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#faf8f5"}}><p>載入中...</p></div>;
...
function Dashboard({auth,onLogout}){
  ...
  const[tab,setTab]=useState(myTabs[0]);
  const[stores,setStores]=useState([]);const[sf,setSf]=useState(auth.role==="store_manager"?auth.store_id||"":"");
  ...
  const load=useCallback(()=>{
    ...
    Promise.all([
      ...
      api(`/api/admin/shifts${sf?`?store_id=${sf}`:""}`),
      api(`/api/admin/schedules?${sp}`),
    ...
    ]).then(([s,d,e,sh,sc,at,as3,lr2])=>{setStl(s.data||[]);setSum(s.summary||{});setDep(d.data||[]);setEmps(e.data||[]);setShifts(sh.data||[]);setScheds(sc.data||[]);setAtt(at.data||[]);setAs2(as3.data||{});setLr(lr2.data||[]);setLd(false);});
    ...
    <div style={{minHeight:"100vh",background:"#faf8f5",fontFamily:"system-ui,'Noto Sans TC',sans-serif"}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"12px 8px"}}>
        <div style={{display:"flex",gap:3,marginBottom:10,overflowX:"auto",paddingBottom:3}}>
          {myTabs.map(id=><button key={id} style={ts(id)} onClick={()=>setTab(id)}>{TAB_LABELS[id]}{id==="leaves"&&pl.length>0&&<span style={{position:"absolute",top:-3,right:-3,background:"#b91c1c",color:"#fff",borderRadius:"50%",width:14,height:14,fontSize:9,display:"flex",alignItems:"center",justifyContent:"center"}}>{pl.length}</span>}</button>)}
        </div>
        ...
        {/* 設定 */}
        {!ld&&tab==="settings"&&<div style={{maxWidth:420}}><div style={{background:"#fff",borderRadius:8,border:"1px solid #e8e6e1",padding:14}}><h3 style={{fontSize:13,fontWeight:500,marginBottom:10}}>⚙️ 打卡設定</h3>{[["late_grace_minutes","遲到寬限(分)"],["late_threshold_minutes","嚴重遲到(分)"],["early_leave_minutes","早退(分)"],["overtime_min_minutes","加班最低(分)"],["work_hours_per_day","每日工時"],["work_hours_per_week","每週工時"]].map(([k,l])=><div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #f0eeea"}}><span style={{fontSize:12}}>{l}</span><input type="number" value={as2[k]??""} onChange={e=>upS(k,Number(e.target.value))} style={{width:55,padding:4,borderRadius:5,border:"1px solid #ddd",textAlign:"center",fontSize:12}}/></div>)}</div></div>}
        ...
        {/* 員工 */}
        {!ld&&tab==="employees"&&<div><button onClick={()=>{setSaf(!saf);setNbc(null);}} style={{padding:"5px 14px",borderRadius:6,border:"1px solid #ddd",background:saf?"#f0f0f0":"#1a1a1a",color:saf?"#666":"#fff",fontSize:11,cursor:"pointer",marginBottom:8}}>{saf?"✕":"＋"}</button>{saf&&<div style={{background:"#fff",borderRadius:8,border:"1px solid #e8e6e1",padding:14,marginBottom:8}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}><div><label style={{fontSize:10,color:"#888"}}>姓名*</label><input value={ne.name} onChange={e=>setNe({...ne,name:e.target.value})} style={{width:"100%",padding:"5px",borderRadius:5,border:"1px solid #ddd",fontSize:11}}/></div><div><label style={{fontSize:10,color:"#888"}}>手機*</label><input value={ne.phone} onChange={e=>setNe({...ne,phone:e.target.value})} placeholder="0912345678" style={{width:"100%",padding:"5px",borderRadius:5,border:"1px solid #ddd",fontSize:11}}/></div><div><label style={{fontSize:10,color:"#888"}}>門市</label><select value={ne.store_id} onChange={e=>setNe({...ne,store_id:e.target.value})} style={{width:"100%",padding:"5px",borderRadius:5,border:"1px solid #ddd",fontSize:11}}><option value="">總部</option>{stores.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div><div><label style={{fontSize:10,color:"#888"}}>角色</label><select value={ne.role} onChange={e=>setNe({...ne,role:e.target.value})} style={{width:"100%",padding:"5px",borderRadius:5,border:"1px solid #ddd",fontSize:11}}><option value="staff">👤 員工</option><option value="store_manager">🏪 門店主管</option><option value="manager">🏠 管理</option><option value="admin">👑 總部</option></select></div></div><button onClick={addEmp} disabled={!ne.name||!ne.phone} style={{padding:"5px 14px",borderRadius:5,border:"none",background:ne.name&&ne.phone?"#0a7c42":"#ccc",color:"#fff",fontSize:11,cursor:"pointer"}}>建立</button>{nbc&&<div style={{marginTop:6,padding:8,background:"#e6f9f0",borderRadius:5,fontSize:12}}><b style={{color:"#0a7c42"}}>綁定碼：{nbc}</b><br/><span style={{fontSize:10}}>LINE 輸入：綁定 {nbc}</span></div>}</div>}<div style={{background:"#fff",borderRadius:8,border:"1px solid #e8e6e1",overflow:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{background:"#faf8f5",borderBottom:"1px solid #e8e6e1"}}>{["姓名","角色","門市","手機","LINE","操作"].map(h=><th key={h} style={{padding:7,textAlign:"left",fontWeight:500,color:"#666"}}>{h}</th>)}</tr></thead><tbody>{emps.map(e=><tr key={e.id} style={{borderBottom:"1px solid #f0eeea",opacity:e.is_active?1:0.4}}><td style={{padding:7,fontWeight:500}}>{e.name}</td><td style={{padding:7}}><RB role={e.role}/></td><td style={{padding:7}}>{e.stores?.name||"總部"}</td><td style={{padding:7,fontSize:10}}>{e.phone||"-"}</td><td style={{padding:7}}>{e.line_uid?<span style={{color:"#0a7c42"}}>✅</span>:<span style={{fontSize:10}}>{e.bind_code||"未綁"}</span>}</td><td style={{padding:7}}>{!e.line_uid&&<button onClick={()=>regen(e.id)} style={{padding:"1px 6px",borderRadius:3,border:"1px solid #ddd",background:"transparent",cursor:"pointer",fontSize:9}}>🔄</button>}</td></tr>)}</tbody></table></div></div>}
      </div>
