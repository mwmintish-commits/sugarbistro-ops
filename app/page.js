      ...
      <div style={{padding:"12px 14px",maxWidth:1200,margin:"0 auto"}}>
          ...
          <div>
            ...
            {(sv==="week"||sv==="biweek") && (() => {
              ...
              const addLv=async(eid,date,lt)=>{
                ...
                if(existing){
                  const hrs=prompt("此日已有排班，請選擇：\n\n輸入請假時數（1~7）= 部分請假（保留排班）\n輸入 0 或取消 = 整天請假（取消排班）");
                  if(hrs===null)return;
                  const h=Number(hrs);
                  if(h>0&&h<8){
                    await ap("/api/admin/schedules",{action:"create",employee_id:eid,store_id:existing.store_id,shift_id:existing.shift_id,date,leave_hours:h,leave_type:lt});
                    load();return;
                  }
                }
        ...
        {schPop && (() => {
          ...
          const schedOnHol = async (eid, sid, shiftName) => {
            ...
            if (popHol) {
              // 國定假日：先記下員工+班別，顯示選擇面板
              setHolPending({ eid, sid, shiftName, storeId });
