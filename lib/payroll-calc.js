...
export async function regeneratePayroll(year, month, opts = {}) {
  ...
  const [schedsAll, otAll, attAll] = await Promise.all([
    supabase.from("schedules")
      .select("employee_id, date, day_type, leave_type, leave_hours, shift_id, break_minutes, notes, shifts(start_time, end_time, break_minutes)")
  ...
  for (const emp of emps || []) {
    ...
    // 「國定假日挪移」判定：notes 包含「挪移」字樣 → 5/1 視為 normal work day（不加給）；5/2 的 holiday_comp 是調整後的國假
    const isTransferred = (s) => (s.notes || "").includes("挪移");

    const workScheds = allScheds.filter(s => ["work","rest_day","national_holiday"].includes(s.day_type));
    const workDays = workScheds.length;
    // 底薪天: 正常工作日 + 挪移國假當天（變成 normal work day）
    const baseDayScheds = workScheds.filter(s => s.day_type === "work" || (s.day_type === "national_holiday" && isTransferred(s)));
    ...
    const premiumDays = new Set(
      allScheds.filter(s => s.day_type === "rest_day" || (s.day_type === "national_holiday" && !isTransferred(s))).map(s => s.date)
    );
    ...
    // 國假上班加給（雙倍）— 排除「挪移」的國假（挪移後 5/2 才是國假，由 paid_leave 補假處理）
    const holScheds = workScheds.filter(s => s.day_type === "national_holiday" && !isTransferred(s));
    let holidayPay = 0;
    ...
    // 兼職 paid_leave 給薪（特休、國定補假等有薪假）
    // 月薪員工 paid_leave 不額外處理（月薪已含全月）
    let paidLeavePay = 0;
    if (!emp.monthly_salary && emp.hourly_rate) {
      for (const s of allScheds) {
        if (s.day_type === "paid_leave") {
          // 預設給 8hr 工資；若 schedule 有 leave_hours 用該值
          const hrs = Number(s.leave_hours) > 0 ? Number(s.leave_hours) : 8;
          paidLeavePay += Math.round(hourlyRate * hrs);
          leaveDetail += (leaveDetail ? "、" : "") + (s.leave_type === "holiday_comp" ? "國定補假" : (LEAVE_LABELS[s.leave_type] || "有薪假")) + (hrs < 8 ? hrs + "hr" : "1天") + "(給薪)";
        }
      }
    }
    ...
    const net = base + otPay + holidayPay + restDayPay + paidLeavePay - ls - hs - suppHealth + allow - deduct - leaveDeduct - lateDeduct;
    ...
    results.push({ name: emp.name, base, otPay, holidayPay, restDayPay, paidLeavePay, ls, hs, suppHealth, allow, deduct, leaveDeduct, lateDeduct, lateMin, earlyMin, net, workDays, actualClockDays, discrepancy });
  }
