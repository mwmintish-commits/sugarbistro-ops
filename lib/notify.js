...
export async function getStoreManagers(sb, store_id, opts = {}) {
  const { includeAdmin = false, includeStoreManager = true, includeAreaManager = true } = opts;
  const client = sb || defaultSb;
  const roles = [];
  if (includeStoreManager) roles.push("store_manager");
  if (includeAreaManager) roles.push("manager");
  ...
  return data.filter(e => {
    ...
    // store_manager 只收所屬門市的事件
    if (e.role === "store_manager") return store_id && e.store_id === store_id;
    // manager / admin 收所有店
    return true;
  });
}
